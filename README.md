# Releases

Public installers and update manifests for RAID3N apps.

## Umbrella Tools

- Latest: **4.6.11**
- Installer: [UmbrellaTools-Setup-4.6.11.exe](https://github.com/xRaid3nx/Releases/releases/download/umbrellatools-v4.6.11/UmbrellaTools-Setup-4.6.11.exe)
- Update manifest: [UmbrellaTools/latest.json](UmbrellaTools/latest.json)

### Modules

| Module | Notes |
|--------|--------|
| Text Expander | Always installed |
| Pulse | Optional — email pace bubble |
| Alarms | Optional |
| Idle Shaker | Optional placeholder |

On app updates, use **Change modules…** in the installer if you want to add or remove modules. **Settings → About** only shows help for installed modules.